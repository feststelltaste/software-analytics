...
def vis_network(nodes, edges, physics=True):
    ...
    html = base.format(id=unique_id, nodes=json.dumps(nodes), edges=json.dumps(edges), physics=json.dumps(True))

    return HTML(html)


def draw(graph, options, physics=True, limit=100, n="n", m="m", r="r"):
    """
    ...
    # somehow it seems that the replacement mechanism of py2neo isn't working
    # let's replace "by hand"
